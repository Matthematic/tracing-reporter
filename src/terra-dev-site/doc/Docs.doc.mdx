...
| types | **true** | a key, value object where the key is the name of a "test type" and the value is the glob that matches those test files |
...
| sortKey | false | the name of the column that each individual table should sort by. e.g. 'name', 'issue', 'link', 'type'. (sorting tables by 'id' is, effectively, choosing not to sort them and thus they will sort in the order they appear in the code) | 'id' |
| sortDirection | false | the direction the sortKey option should follow. One of ['ascending', 'descending'] | 'ascending'
